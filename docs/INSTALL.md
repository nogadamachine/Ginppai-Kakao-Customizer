...
카카오톡 26.7.3 또는 26.8.0 및 iOS 17 이상이 필요합니다. 처음 설치하거나 앱을 업데이트하기 전에 카카오톡의 대화 백업을 확인하세요. 앱을 삭제 / 재설치하면 기존 로그인과 대화가 유지된다고 보장할 수 없습니다.
...
카카오톡 26.7.3 또는 26.8.0의 **본인이 준비한 비암호화 IPA**와 배포 DYLIB가 필요합니다. 이 프로젝트는 IPA를 배포하거나 암호화를 해제하지 않습니다.
...
위 명령에 `--country-ui`를 추가합니다. 코드 지문이 일치하는 26.7.3 또는 26.8.0에만 적용됩니다.
...
- 기능이 안 보임: 카카오톡 버전이 **26.7.3 또는 26.8.0의 검증된 빌드**인지, 트윅 로더와 폴더가 설정됐는지 확인합니다.
- 국가 선택이 회색: 위의 추가 UI 패치가 없는 실행 파일입니다.
- 저장 실패: 미디어를 완전히 연 뒤 다시 시도하고, 사진 추가 권한 / 저장 공간 / 네트워크를 확인합니다. 저장 버튼은 사진 앱으로 바로 저장하며 파일 내보내기는 제공하지 않습니다.
...
3.1.0 정식판은 GitHub Releases와 Ginppai Repo에서 배포합니다. 비탈옥에서는 DYLIB만 넣는 것으로 아래 실행 파일 연결까지 추가되지는 않습니다. 구현된 기능과 남은 시험은 [기능별 검증 범위](FEATURES.md)를 확인하세요.

`--features`는 확인된 KakaoTalk 26.7.3 또는 26.8.0 실행 파일에 메시지 기록, 읽지 않은 수 확장, 서식 전송, 공유 카드 전달, 보조 기기 표시와 게임 탭 설정을 연결하는 옵션입니다. `tools/native_patch.py`와 `tools/native_layouts.py`가 `prepare_ipa.py`와 같은 폴더에 있어야 합니다. 코드 지문이 다르거나 이미 변경된 대상이면 중단합니다. 이 옵션 하나로 모든 추가 기능이 구현되지는 않습니다.
...
탈옥 환경에서는 지원되는 런타임의 함수 연결 API를 사용하도록 준비했습니다. 탈옥 기기 실사용 검증은 아직 수행하지 않았습니다. iPad 트윅은 별도 파일을 그대로 유지하며, 데이터 컨테이너와 계정 설정을 새로 만들지 않습니다.

## 앱 표시 이름 변경

`prepare_ipa.py`에 `--display-name "Ginppai Kakao"`를 추가하면 메인 앱의 포함 언어별 표시 이름을 함께 바꿉니다. 생략하면 원래 이름을 유지합니다. 앱 식별자나 실행 파일 이름은 바뀌지 않습니다. 생성한 IPA를 다시 서명해 설치해야 하며 기존 설치의 설정 화면에서 즉시 바뀌는 옵션은 아닙니다. 현재 파일 변환 시험까지 검증했으며 홈 화면 표시 기기 시험은 남았습니다.
