...
static void saveReport(void) {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, NSEC_PER_SEC), dispatch_get_main_queue(), ^{
        pendingWrite = NO;
        NSDictionary *report = @{ @"version": @"3.0.0", @"build": @KC_BUILD_ID, @"target": @"26.7.3",
...
__attribute__((constructor)) static void initializeKakaoAdBlock(void) {
    @autoreleasepool {
        ...
        NSLog(@"[Ginppai-Kakao-Customizer] 3.0.0 loaded for KakaoTalk 26.7.3 (%lu hooks)", (unsigned long)installed.count);
    }
}
