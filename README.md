...
**nogadamachine 제작** / **3.1.0 정식판** / **KakaoTalk 26.7.3 및 26.8.0 지원**
...
3.0.2는 앱 복귀 후 채팅 탭을 눌러도 친구 목록에 머물던 문제를 수정했습니다. 시작 탭을 실행 중 다시 강제로 바꾸지 않으며, 다른 탭으로 이동할 때 목록을 강제로 맨 위로 옮기지 않습니다. 아이폰 기본 키보드가 iPad 배열로 나오면 **Ginppai-Kakao-iPad 1.1.0**도 함께 업데이트하세요.

3.1.0은 카카오톡 26.8.0의 메시지 처리와 화면 구조에 대응합니다. 숏폼 숨김과 오픈채팅 우선 표시를 새 UI에 맞췄으며, 기존 계정과 트윅 설정을 유지합니다. 정확히 확인한 실행 파일만 연결하고 다른 빌드에는 적용하지 않습니다. [26.8.0 검증 범위](docs/kakao-26.8-support.md)를 확인하세요.
...
3.1.0은 **KakaoTalk 26.8.0 / iOS 26.1 / SideStore + LiveContainer**에서 실행과 주요 회귀 동작을 확인했습니다. 26.7.3의 기존 패치 결과도 바이트 단위로 유지됩니다. 아래 기존 기능의 상세 시험 이력은 버전별 검증 기록에 구분했습니다. 삭제된 텍스트 표시, 메시지 도구, 변경 기록 검색과 설정 복원을 실기에서 검사했습니다. 보관함은 수정 전 원문을 포함한 모든 기록을 검색하고 최근 500개 결과를 표시합니다. 로컬 회귀 검증 188건과 배포 파일의 서명, 구조, 해시 검사를 통과했습니다.
...
지원하는 카카오톡 앱 자체가 **iOS 17 이상**을 요구합니다. 탈옥 여부와 별개로 이 조건을 충족해야 합니다. 이 저장소가 iOS 26의 탈옥을 제공하는 것은 아닙니다. DEB는 일반적인 루트풀 / 루트리스 트윅 설치 구조로 제작했으며 **탈옥 기기 실사용과 SideStore 직접 설치는 아직 검증하지 않았습니다.** 확인한 환경은 iOS 26.1의 SideStore + LiveContainer입니다.
...
DEB/DYLIB만 원래 앱에 설치하면 국가 선택이 비활성화됩니다. `tools/prepare_ipa.py --country-ui`는 코드 지문이 일치하는 26.7.3 또는 26.8.0 실행 파일에 검증된 UI 패치를 적용합니다. 다른 버전이나 이미 변형된 코드에는 적용하지 않습니다. [자세한 방법](docs/INSTALL.md#국가-선택-추가-패치)을 확인하세요.
...
### 3.1.0 - 카카오톡 26.8.0 지원

- 버전과 실행 파일 UUID를 함께 확인해 맞는 네이티브 연결을 선택합니다.
- 메시지 기록, 삭제 원문, 읽음 목록과 입력 표시 판단을 새 실행 파일에 연결합니다.
- 새 숏폼 버튼 구조를 처리하고 오픈채팅 선택을 유지합니다.
- IPA 준비 도구가 두 버전을 지원하며, 암호화 또는 변형된 입력은 중단합니다.
- 설정 하단에 실제 트윅 버전과 카카오톡 버전을 표시합니다.
