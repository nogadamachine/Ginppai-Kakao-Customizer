...
def patch_features(binary):
    ...
    words=flag_load(pc)+[bit_branch(pc+8,pc+20,0,True),0x5400004D,
                         branch(pc+16,UNREAD_RETURN),branch(pc+20,UNREAD_CONTINUE)]
    for i,word in enumerate(words):struct.pack_into('<I',binary,FEATURE_CAVE+4*i,word)
    struct.pack_into('<I',binary,UNREAD_CLAMP-BASE,branch(UNREAD_CLAMP,pc))
    for index,(entry,bit) in enumerate(BOOL_GETTERS):
        ...
        words=flag_load(pc)+[bit_branch(pc+8,pc+20,bit),0x52800020,0xD65F03C0,
    ...
    original=struct.unpack_from('<I',binary,COVER_ENTRY-BASE)[0]
    if original!=COVER_PROLOGUE:raise ValueError('Unexpected Universal cover policy entry.')
    offset=FEATURE_CAVE+32+len(BOOL_GETTERS)*32;pc=BASE+offset
    if offset+28>FEATURE_MARKER:raise ValueError('Native feature header space exhausted.')
    words=flag_load(pc)+[bit_branch(pc+8,pc+20,3),0x52800002,0x52800003,
                         original,branch(pc+24,COVER_ENTRY+4)]
    for i,word in enumerate(words):struct.pack_into('<I',binary,offset+4*i,word)
    struct.pack_into('<I',binary,COVER_ENTRY-BASE,branch(COVER_ENTRY,pc))
    ...
    words=flag_load(pc)+[bit_branch(pc+8,pc+20,3),0x52800020,0xD65F03C0,branch(pc+20,ALIMTALK_GETTER)]
    for i,word in enumerate(words):struct.pack_into('<I',binary,offset+4*i,word)
    for site in ALIMTALK_CALLSITES:
        if struct.unpack_from('<I',binary,site-BASE)[0]!=(branch(site,ALIMTALK_GETTER)|0x80000000):
    ...
    original=struct.unpack_from('<I',binary,MORE_TAB_AVAILABLE-BASE)[0]
    ...
    words=flag_load(pc)+[bit_branch(pc+8,pc+32,4),
        0x12001C10,  # and w16,w0,#0xff
        0x71000A1F,  # cmp w16,#2
        0x54000061,  # b.ne original
        0x52800000,0xD65F03C0,original,branch(pc+36,MORE_TAB_AVAILABLE+4)]
    for i,word in enumerate(words):struct.pack_into('<I',binary,offset+4*i,word)
    struct.pack_into('<I',binary,MORE_TAB_AVAILABLE-BASE,branch(MORE_TAB_AVAILABLE,pc))
...
def patch(data, feature_patches=True):
    ...
    for offset, command, size in commands(binary):
        ...
        padding_end=FEATURE_FLAGS+8 if feature_patches else CALLBACK_SLOT+8
        if va <= CALLBACK_SLOT and padding_end <= va + vm_size:
            if protection & 3 != 3:
            ...
            for index in range(count):
                ...
                if address < padding_end and address + section_size > CALLBACK_SLOT:
    ...
    if fingerprint not in VERIFIED_TEXT_HASHES:
        raise ValueError('Native callbacks support only the verified KakaoTalk 26.7.3 executable.')
    if any(binary[CAVE:MARKER + len(MAGIC)]):
        raise ValueError('Native callback header space is not empty.')
    if struct.unpack_from('<I', binary, SAVE_ENTRY - BASE)[0] != 0xAA1E03E0:
        raise ValueError('Unexpected ChatMessage.save entry instruction.')
    pc = BASE + CAVE
    page_delta = (CALLBACK_SLOT >> 12) - ((pc + 4) >> 12)
    adrp = 0x90000010 | ((page_delta & 3) << 29) | (((page_delta >> 2) & 0x7FFFF) << 5)
    ldr = 0xF9400000 | (((CALLBACK_SLOT & 0xFFF) // 8) << 10) | (16 << 5) | 16
    words = [
        ...
        branch(pc + 32, SAVE_ENTRY + 4),
    ]
    for index, word in enumerate(words):
        struct.pack_into('<I', binary, CAVE + 4 * index, word)
    struct.pack_into('<I', binary, SAVE_ENTRY - BASE, branch(SAVE_ENTRY, pc))
    binary[MARKER:MARKER + len(MAGIC)] = MAGIC
    if feature_patches: patch_features(binary)
    return binary
